...
const service = require('../service/HomecamService');
 ...
 *
 * ⚠️ 30분 제한(1800초) 체크는 "나중에" 서비스에서 duration 계산 후 넣을 것
 * 예)
 *   const dur = await service.getDurationSec(record_no, r_end);
 *   if (dur < 1800) return res.status(400).json({ error: '30분 미만 생성 불가' });
 */
exports.updateEndTime = async (req, res) => {
  ...
  try {
    ...
    const [result] = await endFn(record_no, {
      ...
      duration_sec: duration_sec ?? null, // 서비스/리포지토리에서 GREATEST(?,1) 등 보정 가능
    });
 ...
 * 날짜/키워드 검색
 * query: date(YYYY-MM-DD | YYYYMMDD | MMDD), q ...
 */
...
exports.getHomecamDetail = async (req, res) => {
  ...
  try {
    ...
    return res.json(rows[0]); // ✅ 단건 객체 반환
  } catch (err) {
