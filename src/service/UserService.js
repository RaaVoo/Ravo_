...
const loginUser = async (user_id, user_pw) => {
    ...
    return user;        // 로그인 성공 시 사용자 정보 리턴
};

// 비밀번호 변경 기능
const changeUserPassword = async (user_id, current_pw, new_pw) => {
...
const verificationCodes = {};        // 메일로 온 이메일 인증 코드를 담아놓을 임시 저장소 (인증코드 검증 위함) + 비밀번호 찾기에서도 사용

const sendVerificationEmail = async (u_email, code) => {
...
// 이메일 인증 코드를 '검증'하는 기능
const verifyEmailCode = (u_email, inputCode) => {
...
const requestPhoneVerification = async (user_id, u_name, u_phone) => {
...
const verifyPhoneCode = (u_phone, inputCode) => {
